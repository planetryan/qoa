...
use qoa::vectorization::{
    ...
    apply_rx_vectorized, apply_ry_vectorized, apply_rz_vectorized,
    apply_s_vectorized, apply_swap_vectorized, apply_t_vectorized,
};
...
fn benchmark_single_qubit_gates(group: &mut criterion::BenchmarkGroup<WallTime>, num_qubits: usize, params: &BenchmarkParams) {
    // hadamard gate
    group.bench_function(format!("hadamard_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_hadamard_vectorized(black_box(&mut amps), black_box(params.norm_factor), black_box(0));
        });
    });
    ...
    group.bench_function(format!("rx_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_rx_vectorized(black_box(&mut amps), black_box(0), black_box(params.angle_pi_2));
        });
    });
    ...
    group.bench_function(format!("ry_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_ry_vectorized(black_box(&mut amps), black_box(0), black_box(params.angle_pi_2));
        });
    });
    ...
    group.bench_function(format!("rz_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_rz_vectorized(black_box(&mut amps), black_box(0), black_box(params.angle_pi_2));
        });
    });
    ...
    group.bench_function(format!("phaseshift_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_phaseshift_vectorized(black_box(&mut amps), black_box(0), black_box(params.angle_pi_3));
        });
    });
...
fn benchmark_two_qubit_gates(group: &mut criterion::BenchmarkGroup<WallTime>, num_qubits: usize, params: &BenchmarkParams) {
    // cnot gate
    ...
    group.bench_function(format!("controlled_phase_rotation_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_controlled_phase_rotation_vectorized(black_box(&mut amps), black_box(0), black_box(1), black_box(params.angle_pi_4));
        });
    });
}

// three qubit gate benchmarks
fn benchmark_three_qubit_gates(group: &mut criterion::BenchmarkGroup<WallTime>, num_qubits: usize, _params: &BenchmarkParams) {
    // controlled swap (fredkin) gate
    group.bench_function(format!("controlled_swap_{}_qubits", num_qubits), |b| {
        b.iter(|| {
            let mut amps = initial_state(num_qubits);
            apply_controlled_swap_vectorized(black_box(&mut amps), black_box(2), black_box(0), black_box(1));
        });
    });
}
...
fn system_stress_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("cpu_and_io_stress", |b| {
        b.iter(|| {
            ...
            temp_file.write_all(black_box(&dummy_data)).expect("failed to write to temp file");
        });
    });
