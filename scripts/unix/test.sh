...
set_build_flags() {
    ...
    # Architecture-specific optimizations (using native for testing)
    case $arch in
        x86_64)
            # Try MKL linking first, fallback to native if needed
...
run_tests() {
    ...
    # Try optimized test first, then fallback to generic if needed
    if cargo test --release; then
        ...
        echo ""
        echo -e "\033[1;33m--- optimized test failed, trying with generic target --- \033[0m"
        if RUSTFLAGS="-C target-cpu=generic" cargo test --release; then
            local end_time=$(date +%s)
            local duration=$((end_time - start_time))
            echo ""
            echo -e "\033[1;32m--- tests completed successfully (generic build) --- \033[0m"
            echo "test duration: ${duration} seconds"
        else
            echo ""
...
main() {
    echo "=== QOA unix test script (multi-architecture) ==="
    echo ""
