...
use opencl3;
#[cfg(feature = "opencl")]
// the ffi path has changed, cl_device_type can now be found directly in opencl3::device.
use opencl3::device::CL_DEVICE_TYPE_ALL;

// cl_device_name and cl_device_global_mem_size constants are found directly in opencl3::device.
#[cfg(feature = "opencl")]
use opencl3::device::{CL_DEVICE_NAME, CL_DEVICE_GLOBAL_MEM_SIZE};
// cl_platform_name constant is found directly in opencl3::platform.
#[cfg(feature = "opencl")]
use opencl3::platform::CL_PLATFORM_NAME;


// conditional compilation for mpi.
#[cfg(feature = "mpi")]
use mpi::{traits::*, topology::SystemCommunicator};
...
// Removed: #[cfg(feature = "vulkan")] use crate::vulkan_context_error::VulkanContext;
...
pub fn calculate_partitions(
    ...
    match strategy {
        ...
        PartitionStrategy::MemoryBased => {
            ...
            let min_required_gb = (total_states as f64 * std::mem::size_of::<Complex64>() as f64) / (1024.0 * 1024.0 * 1024.0);
            ...
            if current_total_capacity < min_required_gb {
                ...
                node_memory_capacities_gb.iter_mut().for_each(|c| *c *= scale_factor);
            }

            let states_per_gb = 1.0 / (std::mem::size_of::<Complex64>() as f64 / (1024.0 * 1024.0 * 1024.0));
    ...
    pub fn new() -> Self {
        ...
        #[cfg(feature = "opencl")]
        {
            use opencl3::{
                ...
                device::CL_DEVICE_NAME, // cl_device_name is found directly in opencl3::device.
                device::CL_DEVICE_GLOBAL_MEM_SIZE, // cl_device_global_mem_size is found directly in opencl3::device.
                platform::CL_PLATFORM_NAME // cl_platform_name is found directly in opencl3::platform.
            };
...
impl GpuManager {
    ...
    pub fn new() -> Self {
        ...
        #[cfg(feature = "opencl")]
        {
            // imports necessary constants for opencl.
            use opencl3::{
                device::CL_DEVICE_TYPE_ALL,
                // removed redundant or incorrect imports.
                // device::get_device_info, 
                // platform::get_platform_info, 
                device::CL_DEVICE_NAME, // cl_device_name is found directly in opencl3::device.
                device::CL_DEVICE_GLOBAL_MEM_SIZE, // cl_device_global_mem_size is found directly in opencl3::device.
                        ...
                        let result = if is_gpu_thread && Self::is_gpu_friendly(&work) {
                            match current_backend {
                                ...
                                Some(GpuBackend::Cuda) => {
                                    #[cfg(feature = "cuda")]
                                    {
                                        println!("processing work on cuda gpu: allocating device memory, copying data, launching kernel, copying results.");
                                        Self::process_on_cpu(work) 
                                    }
                                ...
                                Some(GpuBackend::OpenCL) => {
                                    #[cfg(feature = "opencl")]
                                    {
                                        println!("processing work on opencl gpu: creating context/queue/buffers, writing data, compiling kernel, enqueuing, reading results.");
                                        Self::process_on_cpu(work) 
                                    }
                            ...
                            // removed the additional 'Some' wrapper to avoid Option<Option<WorkResult>>.
                            Self::process_on_cpu(work)
                        };
            ...
            GpuBackend::Vulkan => {
                #[cfg(feature = "vulkan")]
                {
                    ...
                    if let Some(_vk_ctx) = vulkan_context { 
                        println!("processing work on vulkan gpu: creating buffers, transferring data, executing compute pipeline, reading results.");
                        match work {
                            WorkItem::StateVectorUpdate { id, .. } => { // added '..'
                            ...
                            WorkItem::GateApplication { id, amplitudes: _, qubit_indices: _, gate_matrix: _ } => {
                            ...
                            WorkItem::ComputeKernel { id, output_size: _, input_data: _, kernel_name: _ } => {
                            ...
                            WorkItem::Measurement { id, qubit_index: _, amplitudes: _ } => { 
        ...
        if self.config.use_mpi {
            #[cfg(feature = "mpi")]
            {
                if let Some(comm) = self.mpi_comm.as_mut() {
                    ...
                    let root_rank = 0;
                    comm.process_at_rank(root_rank).broadcast_into(&mut serialized);
...
impl DistributedRenderer {
    ...
    {
        println!("node {} preparing visualization data for frames {} to {}", 
        ...
        for frame in self.config.start_frame..self.config.end_frame {
            ...
            let quantum_state_data = data_preparer(frame, self.config.width, self.config.height, self.vulkan_context.clone())?;
            
            let serialized_data = serde_json::to_vec(&quantum_state_data)
                .map_err(|e| format!("failed to serialize quantum state data: {}", e))?;

            let output_path = format!("{}/{:04}.{}", self.config.output_dir, frame, self.config.frame_format);
            std::fs::write(&output_path, serialized_data)
                .map_err(|e| format!("failed to write quantum state data to file: {}", e))?;

            let frame_duration = frame_start.elapsed();
            
            println!("node {} prepared visualization data for frame {} in {:.2?}", 
                     self._node_id, frame, frame_duration);
        }
        
        let total_duration = start_time.elapsed();
        println!("node {} completed preparing visualization data for {} frames in {:.2?}", 
    ...
    pub fn merge_and_encode(&self) -> Result<(), String> {
        ...
        println!("initiating video encoding from quantum state data (requires external visualizer/encoder).");
        println!("this step assumes an external tool will consume the generated quantum state data files (e.g., .json) and produce visual frames, which are then encoded into a video.");
        ...
        println!("executing (simulated external visualizer then ffmpeg): {:?}", command);
...
impl DistributedRenderer {
    ...
    pub fn new(
        ...
        width: u32,
        height: u32,
        // vulkan_context: Option<Arc<VulkanContext>>, // removed as it's not available
    ) -> Self {
        let frames_per_node = total_frames / total_nodes;
        let remainder = total_frames % total_nodes;
    ...
    pub fn render_frames<F>(&self, data_preparer: F) -> Result<(), String>
    where
        // F: Fn(usize, u32, u32, Option<Arc<VulkanContext>>) -> Result<Vec<Complex64>, String>, // removed vulkan_context
        F: Fn(usize, u32, u32) -> Result<Vec<Complex64>, String>,
    {
        ...
        for frame in self.config.start_frame..self.config.end_frame {
            let frame_start = Instant::now();
            // let quantum_state_data = data_preparer(frame, self.config.width, self.config.height, self.vulkan_context.clone())?; // removed vulkan_context
...
pub fn process_quantum_state_for_vulkan_visualization(
    ...
    {
        if let Some(_vk_ctx) = vulkan_context { 
            println!("vulkan processing quantum state data for visualization, frame {} ({}x{})", frame_number, width, height);
            let processed_data = vec![Complex64::new(frame_number as f64, 0.0); (width * height / 100) as usize];
            Ok(processed_data) 
        } else {
            println!("vulkan context not available for gpu processing, falling back to cpu processing for visualization.");
            let processed_data = vec![Complex64::new(frame_number as f64, 0.0); (width * height / 100) as usize];
            Ok(processed_data)
        }
    }
    #[cfg(not(feature = "vulkan"))]
    {
        ...
        println!("vulkan feature not enabled, performing cpu processing for visualization, frame {} ({}x{})", frame_number, width, height);
        let processed_data = vec![Complex64::new(frame_number as f64, 0.0); (width * height / 100) as usize];
        Ok(processed_data)
    }
}
...
impl TensorNetwork {
    ...
    pub fn contract_tensors(&mut self, tensor1_id: usize, tensor2_id: usize) -> Result<usize, String> {
        ...
        if !tensor1.data.is_empty() && !tensor2.data.is_empty() {
            for i in 0..new_data.len() {
                let t1_val = tensor1.data.get(i % tensor1.data.len()).unwrap_or(&ZERO_COMPLEX);
                let t2_val = tensor2.data.get(i % tensor2.data.len()).unwrap_or(&ZERO_COMPLEX);
                new_data[i] = t1_val * t2_val;
            }
        }

        if self.max_bond_dimension > 0 {
            println!("note: applying bond dimension truncation to {} based on max_bond_dimension: {}", new_dimensions.len(), self.max_bond_dimension);
        }
    ...
    pub fn apply_single_qubit_gate(&mut self, qubit: usize, gate: [[Complex64; 2]; 2]) -> Result<(), String> {
    ...
    pub fn apply_two_qubit_gate(&mut self, qubit1: usize, qubit2: usize, gate: [[Complex64; 4]; 4]) -> Result<(), String> {
...
impl DistributedSimulator {
    ...
    pub fn new(config: DistributedConfig) -> Result<Self, String> {
        ...
        let quantum_state = crate::runtime::quantum_state::QuantumState::new(
            config.total_qubits, 
            None
        );
    ...
    pub fn run_example_simulation(&mut self) -> Result<(), String> {
        ...
        if let Some(WorkResult::GateApplied { updated_amplitudes, .. }) = result {
            self.quantum_state.set_state_vector(updated_amplitudes)?;
            println!("applied hadamard gate to qubit 0. current state vector size: {}", self.quantum_state.amps.len());
        } else {
    ...
    pub fn run_example_rendering(&self) -> Result<(), String> {
        ...
        renderer.render_frames(|frame, width, height, vk_ctx| {
            println!("preparing quantum state data for frame {} for visualization.", frame);
