...
impl Instruction {
    ...
    pub fn encode(&self) -> Vec<u8> {
        match self {
            // core quantum operations
            Instruction::QINIT(q) | Instruction::QINITQ(q) | Instruction::INITQUBIT(q) | Instruction::QI(q) | Instruction::IQ(q) => vec![0x04, *q],
            Instruction::QMEAS(q) | Instruction::MEASURE(q) | Instruction::MEAS(q) | Instruction::QM(q) | Instruction::M(q) => vec![0x32, *q],
            Instruction::H(q) | Instruction::HAD(q) | Instruction::APPLYHADAMARD(q) => vec![0x05, *q],
            Instruction::CONTROLLEDNOT(c, t) | Instruction::CNOT(c, t) | Instruction::CN(c, t) => vec![0x17, *c, *t],
            ...
            Instruction::RESET(q) | Instruction::RST(q) | Instruction::QRESET(q) | Instruction::QR(q) | Instruction::RSTQ(q) => vec![0x0A, *q], // Changed RT to RSTQ
            Instruction::SWAP(q1, q2) => vec![0x0B, *q1, *q2],
            Instruction::CONTROLLEDSWAP(c, t1, t2) | Instruction::CSWAP(c, t1, t2) | Instruction::CS(c, t1, t2) => vec![0x0C, *c, *t1, *t2],
            Instruction::ENTANGLE(q1, q2) => vec![0x11, *q1, *q2],
            Instruction::ENTANGLEBELL(q1, q2) | Instruction::EBELL(q1, q2) | Instruction::EB(q1, q2) => vec![0x12, *q1, *q2],
            ...
            Instruction::ENTANGLESWAP(a, b, c, d) | Instruction::ESWAP(a, b, c, d) | Instruction::ESP(a, b, c, d) => vec![0x15, *a, *b, *c, *d],
            Instruction::ENTANGLESWAPMEASURE(a, b, c, d, label) | Instruction::ESWAPM(a, b, c, d, label) | Instruction::ESM(a, b, c, d, label) => {
                let mut v = vec![0x16, *a, *b, *c, *d];
                v.extend(label.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::ENTANGLEWITHCLASSICALFEEDBACK(q1, q2, signal) | Instruction::ECFB(q1, q2, signal) | Instruction::EWCFB(q1, q2, signal) => {
                let mut v = vec![0x19, *q1, *q2];
                v.extend(signal.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::ENTANGLEDISTRIBUTED(q, node) | Instruction::EDIST(q, node) | Instruction::ED(q, node) => {
                let mut v = vec![0x1A, *q];
                v.extend(node.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::MEASUREINBASIS(q, basis) | Instruction::MEASB(q, basis) | Instruction::MIB(q, basis) => {
            ...
            Instruction::VERBOSELOG(q, msg) | Instruction::VLOG(q, msg) | Instruction::VL(q, msg) => { // Changed VL pattern
                let mut v = vec![0x87, *q];
                v.extend(msg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::SETPHASE(q, phase) | Instruction::SETP(q, phase) | Instruction::SPH(q, phase) => { // Changed STP to SPH
                let mut v = vec![0x1D, *q];
                v.extend(&phase.to_le_bytes());
                v
            }
            Instruction::APPLYGATE(name, q) | Instruction::AGATE(name, q) | Instruction::AG(name, q) => {
            ...
            // character printing operations
            Instruction::CHARLOAD(reg, val) | Instruction::CLOAD(reg, val) | Instruction::CLD(reg, val) => vec![0x31, *reg, *val],
            Instruction::CHAROUT(reg) | Instruction::COUT(reg) | Instruction::CO(reg) => vec![0x18, *reg],
            ...
            Instruction::THERMALAVG(q, param) | Instruction::TAVG(q, param) | Instruction::TA(q, param) => vec![0x1F, *q, *param],
            Instruction::WKBFACTOR(q1, q2, param) | Instruction::WKBF(q1, q2, param) | Instruction::WF(q1, q2, param) => vec![0x20, *q1, *q2, *param],

            // register set operation
            Instruction::REGSET(reg, val) | Instruction::RSET(reg, val) | Instruction::RGST(reg, val) => {
            ...
            // loop control operations
            Instruction::LOOPSTART(reg) | Instruction::LSTART(reg) | Instruction::LS(reg) => vec![0x01, *reg],
            Instruction::LOOPEND | Instruction::LEND | Instruction::LE => vec![0x10],

            // rotation operations
            Instruction::APPLYROTATION(q, axis, angle) | Instruction::ROT(q, axis, angle) | Instruction::AR(q, axis, angle) => {
                let mut v = vec![0x33, *q, *axis as u8];
                v.extend(&angle.to_le_bytes());
                v
            }
            Instruction::APPLYMULTIQUBITROTATION(qs, axis, angles) | Instruction::MROT(qs, axis, angles) | Instruction::AMQR(qs, axis, angles) => {
            ...
            Instruction::CONTROLLEDPHASEROTATION(c, t, angle) | Instruction::CPHASE(c,t,angle) | Instruction::APPLYCPHASE(c,t,angle) | Instruction::CPR(c,t,angle) => {
            ...
            Instruction::DECOHERENCEPROTECT(q, duration) | Instruction::DPROT(q, duration) | Instruction::DP(q, duration) => {
            ...
            Instruction::LOADMEM(reg, addr) | Instruction::LMEM(reg, addr) | Instruction::LM(reg, addr) => {
                let mut v = vec![0x3D];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
            ...
            Instruction::STOREMEM(reg, addr) | Instruction::SMEM(reg, addr) | Instruction::SM(reg, addr) => {
                let mut v = vec![0x3E];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v.extend(addr.as_bytes());
            ...
            Instruction::LOADCLASSICAL(reg, var) | Instruction::LCL(reg, var) | Instruction::LC(reg, var) => {
                let mut v = vec![0x3F];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
            ...
            Instruction::STORECLASSICAL(reg, var) | Instruction::SCL(reg, var) | Instruction::SC(reg, var) => {
                let mut v = vec![0x40];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v.extend(var.as_bytes());
            ...
            Instruction::JUMP(label) => { // JUMP is now for labels only
                let mut v = vec![0x49];
                v.extend(label.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::JMP(offset) => { // JMP is for i64 offset
                let mut v = vec![0x90];
            ...
            Instruction::CALL(label) | Instruction::CallLabel(label) => { // Use CallLabel for the alias
                let mut v = vec![0x4C];
                v.extend(label.as_bytes());
                v.push(0); // null terminator for string
                v
            ...
            Instruction::TIMEDELAY(q, cycles) | Instruction::TDELAY(q, cycles) | Instruction::TD(q, cycles) => {
            ...
            Instruction::REGADD(rd, ra, rb) | Instruction::RADD(rd, ra, rb) | Instruction::RGA(rd, ra, rb) => vec![0x54, *rd, *ra, *rb],
            Instruction::REGSUB(rd, ra, rb) | Instruction::RSUB(rd, ra, rb) | Instruction::RGS(rd, ra, rb) => vec![0x55, *rd, *ra, *rb],
            Instruction::REGMUL(rd, ra, rb) | Instruction::RMUL(rd, ra, rb) | Instruction::RM(rd, ra, rb) => vec![0x56, *rd, *ra, *rb],
            Instruction::REGDIV(rd, ra, rb) | Instruction::RDIV(rd, ra, rb) | Instruction::RD(rd, ra, rb) => vec![0x57, *rd, *ra, *rb],
            Instruction::REGCOPY(rd, ra) | Instruction::RCOPY(rd, ra) | Instruction::RC(rd, ra) => vec![0x58, *rd, *ra],

            // optics and photonics operations
            Instruction::PHOTONEMIT(q) | Instruction::PEMIT(q) | Instruction::PE(q) => vec![0x59, *q],
            Instruction::PHOTONDETECT(q) | Instruction::PDETECT(q) | Instruction::PD(q) => vec![0x5A, *q],
            Instruction::PHOTONCOUNT(q, reg) | Instruction::PCOUNT(q, reg) | Instruction::PC(q, reg) => {
                let mut v = vec![0x5B, *q];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::PHOTONADDITION(q) | Instruction::PADD(q) | Instruction::PA(q) => vec![0x5C, *q],
            Instruction::APPLYPHOTONSUBTRACTION(q) | Instruction::APSUB(q) | Instruction::APS(q) => vec![0x5D, *q],
            Instruction::PHOTONEMISSIONPATTERN(q, reg, cycles) | Instruction::PEPAT(q, reg, cycles) | Instruction::PEP(q, reg, cycles) => {
            ...
            Instruction::PHOTONDETECTWITHTHRESHOLD(q, thresh, reg) | Instruction::PDTHR(q, thresh, reg) | Instruction::PDWT(q, thresh, reg) => {
            ...
            Instruction::PHOTONDETECTCOINCIDENCE(qs, reg) | Instruction::PDCOIN(qs, reg) | Instruction::PDC(qs, reg) => {
            ...
            Instruction::PHOTONBUNCHINGCONTROL(q, b) | Instruction::PBUNCH(q, b) | Instruction::PBC(q, b) => vec![0x63, *q, *b as u8],
            Instruction::PHOTONROUTE(q, from, to) | Instruction::PROUTE(q, from, to) | Instruction::PR(q, from, to) => {
            ...
            Instruction::OPTICALROUTING(q1, q2) | Instruction::OROUTE(q1, q2) | Instruction::OPTR(q1, q2) => vec![0x65, *q1, *q2],
            Instruction::SETOPTICALATTENUATION(q, att) | Instruction::SOATT(q, att) | Instruction::SOA(q, att) => {
                let mut v = vec![0x66, *q];
                v.extend(&att.to_le_bytes());
                v
            }
            Instruction::DYNAMICPHASECOMPENSATION(q, phase) | Instruction::DPC(q, phase) | Instruction::DPCMP(q, phase) => {
                let mut v = vec![0x67, *q];
                v.extend(&phase.to_le_bytes());
                v
            }
            Instruction::OPTICALDELAYLINECONTROL(q, delay) | Instruction::ODLC(q, delay) | Instruction::ODL(q, delay) => {
                let mut v = vec![0x68, *q];
                v.extend(&delay.to_le_bytes());
                v
            }
            Instruction::CROSSPHASEMODULATION(c, t, stren) | Instruction::CPM(c, t, stren) | Instruction::CPMOD(c, t, stren) => {
                let mut v = vec![0x69, *c, *t];
                v.extend(&stren.to_le_bytes());
                v
            }
            Instruction::APPLYDISPLACEMENT(q, a) | Instruction::ADISP(q, a) | Instruction::AD(q, a) => {
                let mut v = vec![0x6A, *q];
                v.extend(&a.to_le_bytes());
                v
            }
            Instruction::APPLYDISPLACEMENTFEEDBACK(q, reg) | Instruction::ADF(q, reg) | Instruction::ADWF(q, reg) | Instruction::ADFB(q, reg) => {
                let mut v = vec![0x6B, *q];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::APPLYDISPLACEMENTOPERATOR(q, alpha, dur) | Instruction::ADO(q, alpha, dur) | Instruction::ADOP(q, alpha, dur) => {
            ...
            Instruction::APPLYSQUEEZINGFEEDBACK(q, reg) | Instruction::ASF(q, reg) | Instruction::ASWF(q, reg) | Instruction::ASFB(q, reg) => {
                let mut v = vec![0x6E, *q];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::MEASUREPARITY(q) | Instruction::MPAR(q) | Instruction::MP(q) => vec![0x6F, *q],
            Instruction::MEASUREWITHDELAY(q, u64_delay, reg) | Instruction::MWD(q, u64_delay, reg) | Instruction::MWDEL(q, u64_delay, reg) => {
            ...
            Instruction::OPTICALSWITCHCONTROL(q, b) | Instruction::OSC(q, b) | Instruction::OSW(q, b) => vec![0x71, *q, *b as u8],
            Instruction::PHOTONLOSSSIMULATE(q, prob, seed) | Instruction::PLS(q, prob, seed) | Instruction::PLSIM(q, prob, seed) => {
                let mut v = vec![0x72, *q];
                v.extend(&prob.to_le_bytes());
                v.extend(&seed.to_le_bytes());
                v
            }
            Instruction::PHOTONLOSSCORRECTION(q, reg) | Instruction::PLC(q, reg) | Instruction::PLCOR(q, reg) => {
            ...
            // qubit measurement and error correction
            Instruction::APPLYQNDMEASUREMENT(q, reg) | Instruction::AQND(q, reg) | Instruction::AQAD(q, reg) | Instruction::AQNM(q, reg) => {
                let mut v = vec![0x7C, *q];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::ERRORCORRECT(q, syndrome_type) | Instruction::ECORR(q, syndrome_type) | Instruction::EC(q, syndrome_type) => {
                let mut v = vec![0x7D, *q];
                v.extend(syndrome_type.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::ERRORSYNDROME(q, syndrome_type, result_reg) | Instruction::ESYN(q, syndrome_type, result_reg) | Instruction::ES(q, syndrome_type, result_reg) => {
            ...
            Instruction::QUANTUMSTATETOMOGRAPHY(q, basis) | Instruction::QST(q, basis) | Instruction::QSTAT(q, basis) => {
                let mut v = vec![0x7F, *q];
                v.extend(basis.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::BELLSTATEVERIFICATION(q1, q2, result_reg) | Instruction::BSV(q1, q2, result_reg) | Instruction::BSTATE(q1, q2, result_reg) => {
                let mut v = vec![0x80, *q1, *q2];
                v.extend(result_reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::QUANTUMZENOEFFECT(q, num_measurements, interval_cycles) | Instruction::QZE(q, num_measurements, interval_cycles) | Instruction::QZEN(q, num_measurements, interval_cycles) => {
                let mut v = vec![0x81, *q];
                v.extend(&num_measurements.to_le_bytes());
                v.extend(&interval_cycles.to_le_bytes());
                v
            }
            Instruction::APPLYNONLINEARPHASESHIFT(q, strength) | Instruction::ANLPS(q, strength) | Instruction::ANLP(q, strength) | Instruction::ANLPH(q, strength) => {
                let mut v = vec![0x82, *q];
                v.extend(&strength.to_le_bytes());
                v
            }
            Instruction::APPLYNONLINEARSIGMA(q, strength) | Instruction::ANLS(q, strength) | Instruction::ANLSI(q, strength) => {
                let mut v = vec![0x83, *q];
                v.extend(&strength.to_le_bytes());
                v
            }
            Instruction::APPLYLINEAROPTICALTRANSFORM(_name, input_qs, output_qs, _num_modes) | Instruction::ALOT(_name, input_qs, output_qs, _num_modes) | Instruction::ALOPT(_name, input_qs, output_qs, _num_modes) => {
                let mut v = vec![
                    0x84,
                    input_qs.len() as u8,
                    output_qs.len() as u8,
            ...
            Instruction::PHOTONNUMBERRESOLVINGDETECTION(q, reg) | Instruction::PNRD(q, reg) | Instruction::PNR(q, reg) => {
                let mut v = vec![0x85, *q];
                v.extend(reg.as_bytes());
                v.push(0); // null terminator for string
                v
            }
            Instruction::FEEDBACKCONTROL(q, reg) | Instruction::FBC(q, reg) | Instruction::FBCONT(q, reg) => {
            ...
            // miscellaneous operations
            Instruction::SETPOS(q, x, y) | Instruction::SPOS(q, x, y) | Instruction::STP(q, x, y) => {
            ...
            Instruction::WLSHIFT(q, wl_delta) | Instruction::WLS(q, wl_delta) | Instruction::WLSH(q, wl_delta) => {
                let mut v = vec![0x76, *q];
                v.extend(&wl_delta.to_le_bytes());
                v
            }
            Instruction::MOVE(q, dx, dy) | Instruction::MOV(q, dx, dy) | Instruction::MV(q, dx, dy) => {
            ...
            Instruction::MARKOBSERVED(q) | Instruction::MOBS(q) | Instruction::MO(q) => vec![0x79, *q],
        ...
        // new instructions for v0.3.0+
            ...
            Instruction::RETSUB | Instruction::RS => { // Consolidated RETSUB and RS
                vec![0x97]
            }
            ...
            Instruction::ANDBITS(d, o1, o2) | Instruction::ANDB(d, o1, o2) | Instruction::AB(d, o1, o2) => {
                vec![0xA5, *d, *o1, *o2]
            }
            Instruction::ORBITS(d, o1, o2) | Instruction::ORB(d, o1, o2) | Instruction::OB(d, o1, o2) => {
                vec![0xA6, *d, *o1, *o2]
            }
            Instruction::XORBITS(d, o1, o2) | Instruction::XORB(d, o1, o2) | Instruction::XB(d, o1, o2) => {
                vec![0xA7, *d, *o1, *o2]
            }
    ...
    pub fn get_byte_size(&self) -> usize {
        match self {
            ...
            Instruction::COMMENT(text) | Instruction::CMT(text) | Instruction::CM(text) => 1 + text.len() + 1, // opcode + string length + null terminator
            ...
            Instruction::QINIT(_) | Instruction::QINITQ(_) | Instruction::INITQUBIT(_) | Instruction::QI(_) | Instruction::IQ(_) => 2,
            Instruction::QMEAS(_) | Instruction::MEASURE(_) | Instruction::MEAS(_) | Instruction::QM(_) | Instruction::M(_) => 2,
            ...
            Instruction::RESET(_) | Instruction::RST(_) | Instruction::QRESET(_) | Instruction::QR(_) | Instruction::RSTQ(_) => 2, // Changed RT to RSTQ
            ...
            Instruction::APPLYPHOTONSUBTRACTION(_) | Instruction::APSUB(_) | Instruction::APS(_) => 2,
            ...
            Instruction::OPTICALSWITCHCONTROL(_, _) | Instruction::OSC(_, _) | Instruction::OSW(_, _) => 3, // opcode + u8 + bool (1 byte)
            ...
            Instruction::OPTICALROUTING(_, _) | Instruction::OROUTE(_, _) | Instruction::OPTR(_, _) => 3,
            ...
            Instruction::PHOTONBUNCHINGCONTROL(_, _) | Instruction::PBUNCH(_, _) | Instruction::PBC(_, _) => 3,
            Instruction::CHARLOAD(_, _) | Instruction::CLOAD(_, _) | Instruction::CLD(_, _) => 3,

            // 4-byte opcodes (three regs)
            Instruction::CONTROLLEDSWAP(_, _, _) | Instruction::CSWAP(_, _, _) | Instruction::CS(_, _, _) => 4,
            Instruction::WKBFACTOR(_, _, _) | Instruction::WKBF(_, _, _) | Instruction::WF(_, _, _) => 4,
            Instruction::REGADD(_, _, _) | Instruction::RADD(_, _, _) | Instruction::RGA(_, _, _) => 4,
            Instruction::REGSUB(_, _, _) | Instruction::RSUB(_, _, _) | Instruction::RGS(_, _, _) => 4,
            Instruction::REGMUL(_, _, _) | Instruction::RMUL(_, _, _) | Instruction::RM(_, _, _) => 4,
            Instruction::REGDIV(_, _, _) | Instruction::RDIV(_, _, _) | Instruction::RD(_, _, _) => 4,
            Instruction::ANDBITS(_, _, _) | Instruction::ANDB(_, _, _) | Instruction::AB(_, _, _) => 4,
            Instruction::ORBITS(_, _, _) | Instruction::ORB(_, _, _) | Instruction::OB(_, _, _) => 4,
            Instruction::XORBITS(_, _, _) | Instruction::XORB(_, _, _) | Instruction::XB(_, _, _) => 4,
            ...
            Instruction::REGSET(_, _) | Instruction::RSET(_, _) | Instruction::RGST(_, _) => 1 + 1 + 8,
            Instruction::TIMEDELAY(_, _) | Instruction::TDELAY(_, _) | Instruction::TD(_, _) => 1 + 1 + 8,
            Instruction::SETOPTICALATTENUATION(_, _) | Instruction::SOATT(_, _) | Instruction::SOA(_, _) => 1 + 1 + 8,
            Instruction::DYNAMICPHASECOMPENSATION(_, _) | Instruction::DPC(_, _) | Instruction::DPCMP(_, _) => 1 + 1 + 8,
            Instruction::OPTICALDELAYLINECONTROL(_, _) | Instruction::ODLC(_, _) | Instruction::ODL(_, _) => 1 + 1 + 8,
            Instruction::APPLYDISPLACEMENT(_, _) | Instruction::ADISP(_, _) | Instruction::AD(_, _) => 1 + 1 + 8,
            Instruction::APPLYSQUEEZING(_, _) | Instruction::ASQ(_, _) | Instruction::AS(_, _) => 1 + 1 + 8,
            Instruction::DECOHERENCEPROTECT(_, _) | Instruction::DPROT(_, _) | Instruction::DP(_, _) => 1 + 1 + 8,
            ...
            Instruction::WLSHIFT(_, _) | Instruction::WLS(_, _) | Instruction::WLSH(_, _) => 1 + 1 + 8,
            Instruction::SETPHASE(_, _) | Instruction::SETP(_, _) | Instruction::SPH(_, _) => 1 + 1 + 8, // Changed STP to SPH
            Instruction::APPLYNONLINEARPHASESHIFT(_, _) | Instruction::ANLPS(_, _) | Instruction::ANLP(_, _) | Instruction::ANLPH(_, _) => 1 + 1 + 8,
            Instruction::APPLYNONLINEARSIGMA(_, _) | Instruction::ANLS(_, _) | Instruction::ANLSI(_, _) => 1 + 1 + 8,


            // 11-byte opcodes (two u8 + f64)
            Instruction::CONTROLLEDPHASEROTATION(_, _, _) | Instruction::CPHASE(_,_,_) | Instruction::APPLYCPHASE(_,_,_) | Instruction::CPR(_,_,_) => 1 + 1 + 1 + 8,
            Instruction::CROSSPHASEMODULATION(_, _, _) | Instruction::CPM(_, _, _) | Instruction::CPMOD(_, _, _) => 1 + 1 + 1 + 8,
            Instruction::APPLYROTATION(_, _, _) | Instruction::ROT(_, _, _) | Instruction::AR(_, _, _) => 1 + 1 + 1 + 8,
            ...
            Instruction::APPLYDISPLACEMENTOPERATOR(_, _, _) | Instruction::ADO(_, _, _) | Instruction::ADOP(_, _, _) => 1 + 1 + 8 + 8,
            Instruction::PHOTONLOSSSIMULATE(_, _, _) | Instruction::PLS(_, _, _) | Instruction::PLSIM(_, _, _) => 1 + 1 + 8 + 8,
            Instruction::QUANTUMZENOEFFECT(_, _, _) | Instruction::QZE(_, _, _) | Instruction::QZEN(_, _, _) => 1 + 1 + 8 + 8,

            // variable length based on string/vec lengths
            Instruction::APPLYGATE(name, _) | Instruction::AGATE(name, _) | Instruction::AG(name, _) => 1 + 1 + name.len() + 1, // opcode + q + name length + null terminator
            Instruction::ENTANGLEMULTI(qs) | Instruction::EMULTI(qs) | Instruction::EM(qs) => 1 + 1 + qs.len(),
            Instruction::ENTANGLECLUSTER(qs) | Instruction::ECLUSTER(qs) | Instruction::ECR(qs) => 1 + 1 + qs.len(),
            Instruction::ENTANGLESWAP(_, _, _, _) | Instruction::ESWAP(_, _, _, _) | Instruction::ESP(_, _, _, _) => 5,
            Instruction::ENTANGLESWAPMEASURE(_, _, _, _, label) | Instruction::ESWAPM(_, _, _, _, label) | Instruction::ESM(_, _, _, _, label) => 1 + 4 + label.len() + 1,
            Instruction::ENTANGLEWITHCLASSICALFEEDBACK(_, _, signal) | Instruction::ECFB(_, _, signal) | Instruction::EWCFB(_, _, signal) => 1 + 2 + signal.len() + 1,
            Instruction::ENTANGLEDISTRIBUTED(_, node) | Instruction::EDIST(_, node) | Instruction::ED(_, node) => 1 + 1 + node.len() + 1,
            Instruction::MEASUREINBASIS(_, basis) | Instruction::MEASB(_, basis) | Instruction::MIB(_, basis) => 1 + 1 + basis.len() + 1,
            Instruction::VERBOSELOG(_, msg) | Instruction::VLOG(_, msg) | Instruction::VL(_, msg) => 1 + 1 + msg.len() + 1, // Changed VL pattern
            ...
            Instruction::LOADMEM(reg, addr) | Instruction::LMEM(reg, addr) | Instruction::LM(reg, addr) => 1 + reg.len() + 1 + addr.len() + 1,
            Instruction::STOREMEM(reg, addr) | Instruction::SMEM(reg, addr) | Instruction::SM(reg, addr) => 1 + reg.len() + 1 + addr.len() + 1,
            Instruction::LOADCLASSICAL(reg, var) | Instruction::LCL(reg, var) | Instruction::LC(reg, var) => 1 + reg.len() + 1 + var.len() + 1,
            Instruction::STORECLASSICAL(reg, var) | Instruction::SCL(reg, var) | Instruction::SC(reg, var) => 1 + reg.len() + 1 + var.len() + 1,
            Instruction::ADD(dst, src1, src2) => {
            ...
            Instruction::JUMPIFZERO(cond, label) | Instruction::JIZ(cond, label) => 1 + cond.len() + 1 + label.len() + 1,
            Instruction::JUMPIFONE(cond, label) | Instruction::JIO(cond, label) => 1 + cond.len() + 1 + label.len() + 1,
            Instruction::CALL(label) | Instruction::CallLabel(label) => 1 + label.len() + 1,
            Instruction::PHOTONCOUNT(_, reg) | Instruction::PCOUNT(_, reg) | Instruction::PC(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::PHOTONEMISSIONPATTERN(_, reg, _) | Instruction::PEPAT(_, reg, _) | Instruction::PEP(_, reg, _) => 1 + 1 + reg.len() + 1 + 8,
            Instruction::PHOTONDETECTWITHTHRESHOLD(_, _, reg) | Instruction::PDTHR(_, _, reg) | Instruction::PDWT(_, _, reg) => 1 + 1 + 8 + reg.len() + 1,
            Instruction::PHOTONDETECTCOINCIDENCE(qs, reg) | Instruction::PDCOIN(qs, reg) | Instruction::PDC(qs, reg) => 1 + 1 + qs.len() + reg.len() + 1,
            Instruction::PHOTONROUTE(_, from, to) | Instruction::PROUTE(_, from, to) | Instruction::PR(_, from, to) => 1 + 1 + from.len() + 1 + to.len() + 1,
            Instruction::APPLYDISPLACEMENTFEEDBACK(_, reg) | Instruction::ADF(_, reg) | Instruction::ADWF(_, reg) | Instruction::ADFB(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::APPLYSQUEEZINGFEEDBACK(_, reg) | Instruction::ASF(_, reg) | Instruction::ASWF(_, reg) | Instruction::ASFB(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::MEASUREWITHDELAY(_, _, reg) | Instruction::MWD(_, _, reg) | Instruction::MWDEL(_, _, reg) => 1 + 1 + 8 + reg.len() + 1,
            Instruction::PHOTONLOSSCORRECTION(_, reg) | Instruction::PLC(_, reg) | Instruction::PLCOR(_, reg) => 1 + 1 + reg.len() + 1,

            // qubit measurement and error correction
            Instruction::APPLYQNDMEASUREMENT(_, reg) | Instruction::AQND(_, reg) | Instruction::AQAD(_, reg) | Instruction::AQNM(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::ERRORCORRECT(_, syndrome_type) | Instruction::ECORR(_, syndrome_type) | Instruction::EC(_, syndrome_type) => 1 + 1 + syndrome_type.len() + 1,
            Instruction::ERRORSYNDROME(_, syndrome_type, result_reg) | Instruction::ESYN(_, syndrome_type, result_reg) | Instruction::ES(_, syndrome_type, result_reg) => {
                1 + 1 + syndrome_type.len() + 1 + result_reg.len() + 1
            }
            Instruction::QUANTUMSTATETOMOGRAPHY(_, basis) | Instruction::QST(_, basis) | Instruction::QSTAT(_, basis) => 1 + 1 + basis.len() + 1,
            Instruction::BELLSTATEVERIFICATION(_, _, result_reg) | Instruction::BSV(_, _, result_reg) | Instruction::BSTATE(_, _, result_reg) => 1 + 2 + result_reg.len() + 1,
            Instruction::APPLYLINEAROPTICALTRANSFORM(_name, input_qs, output_qs, _num_modes) | Instruction::ALOT(_name, input_qs, output_qs, _num_modes) | Instruction::ALOPT(_name, input_qs, output_qs, _num_modes) => {
                1 + 1 + 1 + 1 + input_qs.len() + output_qs.len()
            }
            Instruction::PHOTONNUMBERRESOLVINGDETECTION(_, reg) | Instruction::PNRD(_, reg) | Instruction::PNR(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::FEEDBACKCONTROL(_, reg) | Instruction::FBC(_, reg) | Instruction::FBCONT(_, reg) => 1 + 1 + reg.len() + 1,
            Instruction::STOREMEMREG(_, _) | Instruction::SMR(_, _) => 1 + 8 + 1,
            Instruction::EXITCODE(_) | Instruction::EXC(_) | Instruction::EX(_) => 1 + 4,

            Instruction::SETPOS(_, _, _) | Instruction::SPOS(_, _, _) | Instruction::STP(_, _, _) => 1 + 1 + 8 + 8,
            Instruction::MOVE(_, _, _) | Instruction::MOV(_, _, _) | Instruction::MV(_, _, _) => 1 + 1 + 8 + 8,

            Instruction::APPLYMULTIQUBITROTATION(qs, _axis, angles) | Instruction::MROT(qs, _axis, angles) | Instruction::AMQR(qs, _axis, angles) => {
    ...
    pub fn parse_instruction(line: &str) -> Result<Instruction, String> {
        ...
        match op {
            ...
            "INITQUBIT" => { // handle INITQUBIT alias
                if str_tokens.len() == 2 {
            ...
            "JMP" => { // This is for relative i64 jumps
                if str_tokens.len() == 2 {
